...
- **Script:** `program/import/impute-stock.R` (new) builds
  ...
  `homes_n`. No `$DATA_PATH` dependency — all inputs already in `derived/`.
...
- **Within-bin year allocation:** MHS state-year `placements`
  (`mhs-state-year.Rds`) for MH, broadcast to every county in the state
  (MHS has no county detail); BPS county-year `permits_sf`
  (`permits-co.Rds`) for site-built, falling back to the state's permit
