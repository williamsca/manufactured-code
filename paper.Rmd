...
The one composition shift that works in the direction of the main result is the elevated-building share, which rises by 6 percentage points for post-1999 construction vintages. Elevation is plausibly part of the treatment channel---the HUD Code encouraged improved siting and installation practices---rather than a confound. Primary-residence share and mandatory purchases are essentially unchanged across vintages.
...
\begin{table}[htbp]
  ...
  \begin{threeparttable}
    ...
    \begin{tablenotes}
      \item Notes: Coefficients from Equation \eqref{eq:event-study-benefit} estimated on claim-level data (outcome: building damage, in thousands of 2000 dollars). Column (2) uses county $\times$ loss-period fixed effects (baseline, as in Table \ref{tab:claims-outcomes}). Column (1) coarsens to state $\times$ loss-period; column (3) refines to census tract $\times$ loss-period. Source: OpenFEMA claims data.
    \end{tablenotes}
  \end{threeparttable}
\end{table}
\end{landscape}
